...
1. A `POST` matching a configured mint path or suffix uses the existing JWT authentication sources and returns `{ticket, expires_in}`. Redis stores the ticket hash plus an HMAC-authenticated envelope containing the expiry and validated user, company, project-role, and matched-consumer context. It does not store the JWT.
2. A WebSocket upgrade with `?ticket=…` atomically retrieves and deletes the ticket with Redis `GETDEL`, verifies the envelope MAC and expiry, restores the same backend-facing identity and consumer context as JWT authentication, removes the ticket from the upstream query, and proxies the upgrade.
...
Identity and roles are a snapshot from the mint request and remain valid only for the ticket TTL. Unsigned records and records copied to another ticket key are rejected.
...
| `ticket_debug_logging` | `false` | Emit presence-only ticket flow milestones at info level. |
| `ticket_param_name` | `ticket` | Query parameter for the opaque ticket. |
| `ticket_mint_paths` | `[]` | Exact paths answered by the plugin for minting. At least one exact path or suffix is required when enabled. |
...
| `redis_host` / `redis_port` | — / `6379` | Single-node or replication endpoint. `redis_host` is required outside cluster mode. |
...
| `redis_database` | `0` | Logical database. Must be `0` in cluster mode. |
...
Set `ticket_debug_logging: true` temporarily to trace ticket request selection, identity fields found, Redis write/consume milestones, record validation, consumer restoration, header restoration, and upstream handoff. These logs contain no field values, counts, hashes, tickets, JWTs, Redis records, identifiers, endpoints, or credentials.

Redis Cluster mode also requires this Nginx shared dictionary on every Kong gateway:
...
If that environment variable already defines a dictionary, append the full directive in the same value:
...
config:
  ...
  redis_cluster_name: tenant-ws-ticket
  redis_cluster_nodes:
    - host: redis-cluster-0.redis-headless.example.svc.cluster.local
      port: 6379
    - host: redis-cluster-1.redis-headless.example.svc.cluster.local
      port: 6379
...
Provision `ticket_record_secret` before upgrading ticket-enabled tenants to `2.8.0`; plugin validation fails without it. Versions 2.7.x and 2.8.0 cannot consume each other's ticket records, so roll all gateway pods together and expect tickets minted during the short mixed-version window to fail. For Redis Cluster, roll out the gateway shared dictionary first and enable cluster mode last. Verify ticket mint and consume before migrating clients from `?token=` to `?ticket=`. Redact the ticket query parameter in ingress, WAF, and tracing logs; the plugin strips it only from the upstream request.
...
| `ws_ticket_forged` | Redis ticket record is unsigned, has an invalid MAC, or was copied to another key |
| `ws_ticket_expired` | Authenticated Redis ticket record expiry has passed |
